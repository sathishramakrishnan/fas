...
function varargout = FusionAnalyzerContent(varargin)
% FUSIONANALYZERCONTENT MATLAB code for doing lipid mixing analysis
% FUSIONANALYZERCONTENT, by itself, creates a new FUSIONANALYZERCONTENT 
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @FusionAnalyzerContent_OpeningFcn, ...
                   'gui_OutputFcn',  @FusionAnalyzerContent_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before fusionanalyzercontent is made visible.
function FusionAnalyzerContent_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle upto figure
% eventdata  reserved - upto be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments upto fusionanalyzercontent (see VARARGIN)

% Choose default command line output for fusionanalyzercontent
...
% tgroup = uitabgroup(handles.fusionanalyzercontent,'Position',pos1);
...
% UIWAIT makes fusionanalyzercontent wait for user response (see UIRESUME)
% uiwait(handles.fusionanalyzercontent);


% --- Outputs from this function are returned upto the command line.
function varargout = FusionAnalyzerContent_OutputFcn(hObject, eventdata, handles) 
...
function fusionanalyzercontent_CreateFcn(hObject, eventdata, handles)
% hObject    handle upto fusionanalyzercontent (see GCBO)
% eventdata  reserved - upto be defined in a future version of MATLAB
...
% --- Executes on button press in loadcontent.
function loadcontent_Callback(hObject, eventdata, handles)
% hObject    handle to loadcontent (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[fileName,pathName] = uigetfile({'*.avi';'*.*'},'Open Movie');
if (fileName==0)
...
close(h) 
set(handles.contentfilename,'String',fileName); 
if isstruct(Movie)
    handles.Content=Movie;
    maxframe=size(handles.M);  
break; 
end
end
...
% Check the file is loaded into handles
TF = isempty(handles.Content);
if (TF == 0)
    set(handles.status,'String','Content File Loaded')
end
set(handles.loadcontent,'string','Loaded','enable','on','BackgroundColor','green');
guidata(hObject, handles);
...
subContent = imcrop(ContentImage, dimensions);
...
% subContentImage2 = imcrop(subContent,[X1,Y1,abs(X2-X1),abs(Y2-Y1)]);
Hole(i).cdata = subImage;
Hole(i).colormap = [];
ContentHole(i).cdata = subContent;
ContentHole(i).colormap = [];
end
handles.hole = Hole;
handles.Contenthole = ContentHole;
...
% Import thr desired hole movie to process
finalmovie = handles.hole;
finalcontentmovie = handles.Contenthole;
moviesize = size(finalmovie);
% Apply Average and Median Filters to smoothen the noise further
h = ones(3,3)/9;
for i = 1:moviesize(2)
...
%   time = time*i;
    IMA = double(analyzemovie(:,:,i)); % Number the image
    cIMA = double(finalcontentmovie(i).cdata);
    IMA(IMA<20)=0;
    bw = IMA>50; % Threshold value
    bwsum = sum(sum(bw)); % Total number of bright pixels
    img = bw.*IMA; % Masked image    
...
time = [acqtime:acqtime:totaltime];
I_imgDiff = I_imgSum - I_ringIn;
cI_imgDiff = I_cimgSum - cI_ringIn;
order = 2;
framelen = 7;
sgf = sgolayfilt(I_ringIn,order,framelen);
sgf2 = sgolayfilt(I_imgDiff,order,framelen);
...
for i =1:z
I_ringInNorm(i) =(I_ringIn(i)-min(I_ringIn))/(max(I_ringIn)-min(I_ringIn));
cI_ringInNorm(i) =(cI_ringIn(i)-min(cI_ringIn))/(max(cI_ringIn)-min(cI_ringIn));
I_diffNorm(i) = (I_imgDiff(i)-min(I_imgDiff))/(max(I_imgDiff)-min(I_imgDiff));
end
%Plot the Normalized Intensities
axes(handles.a6);
refreshdata
sgfn = sgolayfilt(I_ringInNorm,order,framelen);
graph1 = plot(time,sgfn,'b');
set(graph1,'LineWidth',2);
hold on
graph2 = plot(time,cI_ringInNorm,'g');
hold on
graph3 = plot(time,I_diffNorm,'--r');
hold off
set([graph1,graph2,graph3],'LineWidth',2);
legend('In','Calcium','Out');
xlabel('Time(s)')
ylabel('Normalized Intensity')
preference = 1;
guidata(hObject,handles)
